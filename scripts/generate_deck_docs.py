...
STATE_ICON = {
    "stable": ":material-check-circle:{ .success }",
    "active": ":material-progress-wrench:{ .info }",
    "wip": ":material-alert-circle:{ .warning }",
}
...
DECK_TYPE_ICON = {
    "Loupedeck Live": ":material-dialpad:",
    "Stream Deck XL": ":material-grid:",
    "Stream Deck Original": ":material-grid:",
    "Stream Deck +": ":material-grid:",
    "X-Touch Mini": ":material-knob:",
}
...
def build_index(aircraft_entries: list[dict[str, Any]]) -> str:
    ...
    for entry in aircraft_entries:
        ...
        card.append("")
        if thumbnail:
            card.append(f"    ![{title} preview]({thumbnail}){{ loading=lazy }}")
            card.append("")
        if eyebrow:
            card.append(f"    :material-airplane-marker: *{eyebrow}*")
            card.append("")
        card.append(f"    {summary}")
        card.append("")
        # Status and stats line
        meta_parts: list[str] = []
        if state:
            icon = STATE_ICON.get(state, "")
            label = STATE_LABEL.get(state, state.title())
            meta_parts.append(f"{icon} {label}" if icon else label)
        if layout_count:
            meta_parts.append(f"{layout_count} layout{'s' if layout_count != 1 else ''}")
        if page_count:
            meta_parts.append(f"{page_count} pages")
        if meta_parts:
            card.append(f"    {' · '.join(meta_parts)}")
            card.append("")
...
def main() -> int:
    ...
    for aircraft_dir in sorted(path for path in DECKS_DIR.iterdir() if path.is_dir()):
        ...
        # Find home page thumbnail from first layout
        thumbnail = ""
        if layouts and page_images:
            first_layout = layouts[0]
            first_images = page_images.get(first_layout["layout"], {})
            if first_images:
                first_page = next(iter(first_images))
                thumb_path = first_images[first_page]
                thumbnail = f"../assets/images/{slug}/generated/{first_layout['layout']}/{thumb_path.name}"
